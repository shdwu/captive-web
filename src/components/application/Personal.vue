  ...
  <div class="info">
    ...
    <div class="info_right">
      ...
      <div class="info_list">
        ...
        <el-button class="el_btn" type="primary" @click="dialogFormVisiblet = true">绑定邮箱</el-button>
      </div>
      <div class="info_list">
        <span class="info_list_left">修改密码 </span>
        <span class="info_list_centen">密码要求至少包含字母，符号或数字重的两项且长度超过
          8位，建议您经常修改密码，以保证帐号更加安全。</span>
        <el-button class="el_btn" type="primary" @click="dialogFormVisibles = true">修改密码</el-button>
      </div>
    </div>
    ...
    <!-- 绑定手机号的模态框-->
    <el-dialog title="绑定手机号" :visible.sync="dialogFormVisible" width="40%">
      <el-form :model="form">
        <el-form-item label="手机号" :label-width="formLabelWidth">
          <el-input v-model="form.name" autocomplete="off" class="inputs"></el-input>
        </el-form-item>
        <el-form-item label="验证码" :label-width="formLabelWidth">
          <el-input v-model="form.region" autocomplete="off" class="inpu_yzm"></el-input>
    ...
    <!-- 绑定邮箱的模态框 -->
    <el-dialog title="绑定邮箱" :visible.sync="dialogFormVisiblet" width="40%">
      <el-form :model="formEmail" ref="dynamicValidateForm">
        <el-form-item label="邮箱" prop="email" :rules="[
      { required: true, message: '请输入邮箱地址', trigger: 'blur' },
      { type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change'] }
    ]" :label-width="formLabelWidth">
          <el-input v-model="formEmail.email" class="inputs"></el-input>
        </el-form-item>

        <el-form-item label="验证码" :label-width="formLabelWidth">
          <el-input v-model="formEmail.validCode" class="inpu_yzm"></el-input>
          <el-button :type="butSyysf===false?'success':'info'" :disabled="disableds" ref="btnYs"
            @click="sendVerification">{{emailYzm}}</el-button>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisiblet = false">取 消</el-button>
        <el-button type="primary" @click="upDateEmil('dynamicValidateForm')">确 定</el-button>
      </div>
    </el-dialog>
    <!-- 修改密码的模态框 -->
    <el-dialog title="修改密码" :visible.sync="dialogFormVisibles" width="40%">
      <el-form :model="modifyForm" :rules="rules" ref="upDataPassword">
        <el-form-item label="旧密码" prop="originPassword" :label-width="formLabelWidth">
          <el-input v-model="modifyForm.originPassword" type="password" class="inputs"></el-input>
        </el-form-item>
        <el-form-item label="新密码" prop="newPassword" :label-width="formLabelWidth">
          <el-input v-model="modifyForm.newPassword" type="password" class="inputs"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="newPasswords" :label-width="formLabelWidth">
          <el-input v-model="modifyForm.newPasswords" type="password" class="inputs"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="updatedList('upDataPassword')">取 消</el-button>
        <el-button type="primary" @click="updatePassword('upDataPassword')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
