  ...
  <div class="info">
    ...
    <div class="info_title">
      <div class="info_ss">
        ...
        <el-date-picker v-model="searchDate" type="daterange" size="small" range-separator="至" start-placeholder="开始日期"
          end-placeholder="结束日期">
        </el-date-picker>
        <el-button type="primary" size="mini" @click="getTableData()" style="margin-left: 10px;" plain>查询</el-button>
        <el-button type="primary" size="mini" @click="searchExport()" plain>导出</el-button>
      </div>

      <el-table :data="tableData" :empty-text="emptyText" v-loading="loading" style="width: 100%;cursor:pointer;">
        <el-table-column prop="dateExt" label="日期">
        </el-table-column>
        <el-table-column prop="mmsi" label="mmsi">
        </el-table-column>
        <el-table-column prop="shipEn" label="异常船舶">
        </el-table-column>
      </el-table>

      <el-pagination background class="fl" layout="prev, pager, next" @size-change="getTableData"
        @current-change="handleCurrentChange" :current-page="page" :page-size="pagesize" :total="total">
      </el-pagination>
    </div>
  </div>
</template>
